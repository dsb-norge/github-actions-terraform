...
#      [provider-set] [provider-set-environments-json] [provider-set-lock] [root-kind] [name]
# <name> defaults to the engine's 'Terraform test (<file>)', without a set suffix.
mrow() {
  jq -nc --arg slug "${1}" --arg file "${2}" --arg root "${3}" --arg lane "${4}" \
    ...
    --arg name "${11:-Terraform test (${2})}" '
    {slug: $slug, test: {file: $file, name: $name, root: $root, rel: ($file | ltrimstr($root + "/")), lane: $lane,
...
mrow "root--unit-app--a1b2c3" "tests/unit-app.tftest.hcl" "." "unit" false "" "a1b2c3" '["prod"]' "envs/prod/.terraform.lock.hcl" \
  repo-root "Terraform test (tests/unit-app.tftest.hcl) [providers: prod]"
mrow "root--unit-app--d4e5f6" "tests/unit-app.tftest.hcl" "." "unit" false "" "d4e5f6" '["dev","staging"]' "envs/dev/.terraform.lock.hcl" \
  repo-root "Terraform test (tests/unit-app.tftest.hcl) [providers: dev, staging]"
...
# ----------------------------------------------------------------------
setup
mrow "modules-app--unit-app" "modules/app/tests/unit-app.tftest.hcl" "modules/app" "unit" false "" "a1b2c3" '["prod"]'
mrow "envs-prod--unit-env" "envs/prod/tests/unit-env.tftest.hcl" "envs/prod" "unit" false "" "" '[]' "" environment
mrow "modules-app--other--a1b2c3" "modules/app/tests/other.tftest.hcl" "modules/app" "default" false "" "a1b2c3" '["prod"]' \
  "envs/prod/.terraform.lock.hcl" module "Terraform test (modules/app/tests/other.tftest.hcl) [providers: prod]"
mrow "modules-app--other--d4e5f6" "modules/app/tests/other.tftest.hcl" "modules/app" "default" false "" "d4e5f6" '["dev"]' \
  "envs/dev/.terraform.lock.hcl" module "Terraform test (modules/app/tests/other.tftest.hcl) [providers: dev]"
meta "modules-app--unit-app" pass "" 2 0 0 0 1000
meta "envs-prod--unit-env" pass "" 1 0 0 0 1000
meta "modules-app--other--a1b2c3" pass "" 2 0 0 0 1000
meta "modules-app--other--d4e5f6" pass "" 2 0 0 0 1000
job 901 "tf / Terraform test (modules/app/tests/unit-app.tftest.hcl)" success "${T0}" "2026-09-25T10:00:10Z" 14
job 902 "tf / Terraform test (envs/prod/tests/unit-env.tftest.hcl)" success "${T0}" "2026-09-25T10:00:10Z" 14
job 903 "tf / Terraform test (modules/app/tests/other.tftest.hcl) [providers: prod]" success "${T0}" "2026-09-25T10:00:10Z" 14
job 904 "tf / Terraform test (modules/app/tests/other.tftest.hcl) [providers: dev]" success "${T0}" "2026-09-25T10:00:10Z" 14
write_jobs
run_step
assert "narrowed-sets: an environment root's empty set is not counted" \
  body_has "· 2 provider sets ·"
assert "narrowed-sets: a narrowed lane's unsuffixed job is found by the row's name" \
  body_has "[job log](${URL}/job/901#step:14:1)"
assert "narrowed-sets: an environment root's job is found" \
  body_has "[job log](${URL}/job/902#step:14:1)"
assert "narrowed-sets: each set's suffixed job is found" \
  bash -c "grep -qF '/job/903#step:14:1' '${BODY_FILE}' && grep -qF '/job/904#step:14:1' '${BODY_FILE}'"
teardown
