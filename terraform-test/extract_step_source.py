...
rather than a copy of it (the legacy terraform-test action, before its
conversion to the modern layout). GitHub Actions expressions are substituted with
    ...
    for substitution in substitutions:
        ...
        if value.startswith("@"):
            with open(value[1:], encoding="utf-8") as handle:
                value = handle.read()
        # Any spacing inside the braces: the legacy terraform-test block has
        # both "${{ inputs.test-file }}" and "${{ inputs.test-file}}".
        pattern = re.compile(r"\$\{\{\s*" + re.escape(key) + r"\s*\}\}")
        source = pattern.sub(lambda _match: value, source)

    leftover = re.search(r"\$\{\{[^}]*\}\}", source)
    if leftover:
        print(f"unsubstituted expression: {leftover.group(0)}", file=sys.stderr)
        return 1
