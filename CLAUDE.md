...
The spine of the default workflow is the **decision engine** in `engine/` (`docs/Decision-engine.md`): a Python 3.12+ standard-library package, one JSON document in and one out, under a 100 percent line and branch coverage gate and a mutation gate. It has two sides. **The core** (`model`, `values`, `environments`, `decide`, `record`) is pure: it imports only `json` and itself, never YAML, the network, the filesystem or the environment (`test_purity.py` enforces it). **The adapter side** (`adapter.py`, `workflow.py`, `__main__.py`) does the rest: `create-tf-vars-matrix`'s whole step is `python3 -I -B engine/run.py create-matrix --inputs-file <file>`, and the adapter probes `yq`, parses every `*-yml` value, reads the default branch from `GITHUB_EVENT_PATH`, checks directories, decides in-process and publishes `matrix-json`. Every future decision (relevance, tests, dispatch, ordering) is a rule in the core; its fact-gathering is adapter-side Python, not bash (D13).
