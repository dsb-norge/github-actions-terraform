...
| P33 | **The sweep deleted nothing at all, silently.** `DELETE repos/<repo>/git/${ref#refs/}` addresses `git/tags/<name>` — the tag-*object* endpoint — instead of `git/refs/tags/<name>`, so every call 404s; `\|\| echo "(already gone?)"` then swallowed it and the job stayed green. Found 2026-09-17 by a throwaway pull request opened for exactly this, after the tags it published outlived it. The upstream implementation this was ported from had it too, with 22 orphan tags and no open pull requests to explain them. | The ref is appended whole, a 404 is distinguished from a real failure by re-reading the ref, and the step ends by re-listing matching refs and failing if any survived. Verified by opening and closing a probe pull request. |
...
- **Consumed from a calling repo (2026-09-17).** With the App configured, the introducing PR's own `preview/pr-<N>` was consumed by a calling repo's throwaway environment: every `Download action repository` line in its `Set up job` logs named the pinned tag `preview/pr-<N>-<sha7>` — none `@v0`, none the old dev tag — and the failed-apply shapes of the reporting spec reproduced unchanged. The manual swap commit was then dropped from the branch. Dropping it replays every later commit that touched lines next to the swapped `uses:` lines (one conflict here); resolve from the replayed commit's side and put the refs back with an inline `sed`. The rewrite script does **not** exist yet at that point in history, so calling it does nothing and the swap silently survives inside the replayed commit — audit the tip with a single concatenated `grep -c` (per-file `grep -c` prints one count per file, which a naive sum swallows) before pushing.

<!-- probe 2: re-verifies the cleanup sweep after the P33 fix; deleted immediately -->
