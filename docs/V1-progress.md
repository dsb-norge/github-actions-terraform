...
| Terraform-tests.md | isolation end to end; a real environment lane; case in flexible credential `matches`; Dependabot runs and OIDC; fork runs and environment creation; the copied lock and the runner's platform; the shared plugin-cache key | open (spec §12): the first three need Entra and Azure, the next a Dependabot pull request, fork creation is blocked by the organisation's fork policy for private repositories, two are design choices for the maintainer |
