...
| Terraform-tests.md | isolation end to end; a real environment lane; case in flexible credential `matches`; Dependabot runs and OIDC; fork runs and environment creation | open (spec §12): the first three need Entra and Azure (a session with the maintainer in the prod tenant's sandbox subscription is planned), the next a Dependabot pull request, fork creation is blocked by the organisation's fork policy for private repositories |
