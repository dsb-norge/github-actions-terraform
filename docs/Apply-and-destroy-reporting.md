...
- **Ordering inside the real workflow.** F5/F5c grep the YAML; they do not prove the runtime behaviour. `action-tests.yml` runs action suites, not the reusable workflow. Verification is manual, via the dev-tag swap flow ([Development-and-release.md](Development-and-release.md)) against a calling repo with `apply-on-pr`, checking that a **deliberately failed** apply still updates the head comment.
